...
playsound3.playsound('background_music.mp3', False)
...
class Game:
    def __init__(self):
        ...
        self.name = random.choice(os.listdir("C:\\Users\Iron\Desktop\Новая папка\pythonProject\data"))
        ...
        while self.running:
            ...
            self.clock.tick(30)
            if self.balance < 0:
                os.startfile('end.mp4')
                break

    def load_image(self, name, images=False):
        if images:
            fullname = os.path.join('images.png', name)
        else:
    ...
    def BackGround(self):
        back = self.load_image('background.png')
        self.screen.blit(back, (0, 0))
        T1, T2 = self.load_image('39.png', images=True), self.load_image('52.png', images=True)
        T1 = pygame.transform.rotate(T1, 30)
        T2 = pygame.transform.rotate(T2, 35)

        many_f = self.load_image('многа фишек.png', images=True)
        many_f = pygame.transform.scale(many_f, (many_f.get_width() * 1.5, many_f.get_height() * 1.5))

        deck_of_card = self.load_image('карты.png', images=True)
        deck_of_card = pygame.transform.scale(deck_of_card, (deck_of_card.get_width() * 2, deck_of_card.get_height() * 2))

        rol = self.load_image('roulet.png', images=True)

        hand = self.load_image('hand.png', images=True)
        hand = pygame.transform.scale(hand, (hand.get_width() * 1.5, hand.get_height() * 1.5))

        phone = self.load_image('phone.png', images=True)
    ...
    def cards_swap(self):
        k = random.choice(os.listdir("C:\\Users\Iron\Desktop\Новая папка\pythonProject\data"))
